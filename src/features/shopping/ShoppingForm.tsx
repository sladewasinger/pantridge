...
export function ShoppingForm({ item, onClose }: { item?: ShoppingItem; onClose: () => void }) {
  ...
  function setName(name: string) {
    ...
    setDraft(match ? { ...rest, name, foodId: match.id, unit: match.unit } : { ...rest, name });
  }
  return (
    <Modal title={item ? 'Edit shopping item' : 'Add to your list'} onClose={onClose}>
      ...
      >
          ...
          <div className="suggestions" aria-label="Remembered foods">
            {suggestions.map((food) => (
              <button
                ...
                key={food.id}
                onClick={() =>
                  setDraft({ ...draft, name: food.name, unit: food.unit, foodId: food.id })
                }
              >
        ...
        {draft.foodId && <p className="linked-note">Linked to your kitchen inventory</p>}
        <label>
          Unit
          <select
            value={draft.unit}
