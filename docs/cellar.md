...
The home kitchen floor meets a continuous underground section through an 80px dark brown soil band. The wooden hatch scrolls to that section and moves keyboard focus there; normal page scrolling works too. Roots, soil colors, stones and shelf finishes deepen along the page. Reduced-motion users get an immediate jump.

The cellar displays the same stocked food records as the pantry, fridge and freezer, never duplicate inventory. Groups sort Unspecified, Fridge, Pantry, then Freezer; each group's items sort alphabetically, with ID as a stable tie-breaker. `location: "unspecified"` is the catchall for household supplies. Location icons identify each tile. Freezer compatibility remains `location: "fridge", frozen: true`. Zero-stock identities remain available to search and shopping but disappear from all shelves.
