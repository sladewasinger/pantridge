...
Quality also runs WebKit on macOS. Its offline resilience test shuts down a dedicated server after the production service worker caches the app, then verifies shopping, put-away, artwork and repeated reloads. Chromium retains the full simulated device-disconnection suite and CDP touch test. WebKit's `setOffline` produced internal errors even with a minimal independent service worker, while serving cached content after a real server shutdown worked. Windows WebKit also flattened 3D rendering that passed on macOS; it is not treated as iPhone rendering evidence.
